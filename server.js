  ...
  if (req.url === '/audit' && req.method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        ...
        const { instrument, direction, entry, sl, tp, reason, emotion, checklist } = data;
        ...
        if (!apiKey) {
          ...
          res.end(JSON.stringify({ error: 'API key not configured. Please set GEMINI_API_KEY environment variable.' }));
          return;
        }
